package com.example.openwrtmanager.com.example.openwrtmanager.ui.network_traffic.adapter
...
class NetworkTrafficAdapter : ListAdapter<MutableList<InfoResponseModelItem>, RecyclerView.ViewHolder>(
    InfoDiffCallback()
) {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        Log.d("DEBUG", "onCreateViewHolder: "+viewType)
        val itemBinding = ListItemNetworkTrafficBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return IpNetworkTrafficHolder(itemBinding)
    }

    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        Log.d("DEBUG", "currentList123: "+currentList[position][0])
        (holder as IpNetworkTrafficHolder).bind(currentList[position][0],currentList[position][1])
    }
}
